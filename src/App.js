...
class App extends Component {
  ...
  addVisitedCountry = id => {
    ...
    const visitCountry = countriesList.map(eachCounty => {
      if (eachCounty.id === id) {
        const updatList = {...eachCounty, isVisited: !eachCounty.isVisited}
        return updatList
      }
      return eachCounty
    })
  ...
  render() {
    const {countriesList} = this.state
    const updatList = countriesList.filter(
    ...
    return (
      <div className="app-container">
        ...
        <div>
          {updatList.length > 0 ? (
            <ul className="image-remove-container">
              {updatList.map(each => (
                <VisitImage
                  imageDetails={each}
                  key={each}
                  deletevisitCountries={this.deletevisitCountry}
                />
              ))}
            </ul>
          ) : (
